...
const navigate=useNavigate()
    ...
    const handleClick = async () => {
        ...
        const response = await fetch(`http://localhost:4000/bookbandits/collection/${id}`, {
            method: "POST",
            body: JSON.stringify({ borrowed: !fetchedBook.borrowed, owner: fetchedBook.owner,btime: btime, title: fetchedBook.title }),
            headers: {
    ...
    return (
        fetchedBook && (
            <div className=' mb-2 bg-white p-3 shadow-lg rounded cursor-pointer transition-colors border-b-2 border-transparent hover:border-pink-500 text-gray-700 w-72'>
                ...
                {!fetchedBook.borrowed ? !btime ?  <Datetime input={false} onChange={(e) => {setBtime(e._d)}}/> : <button onClick={handleClick} className='bg-white bg-opacity-60 px-6 py-2 border-2 border-white-500  font-medium text-xs leading-tight uppercase rounded-full hover:bg-pink-600 hover:bg-opacity-[45%] focus:outline-none focus:ring-0 transition duration-150 ease-in-out cursor:pointer'>Now book your Book!</button> :<button  className=' bg-white bg-opacity-60 px-6 py-2 border-2 border-white-500  font-medium text-xs leading-tight uppercase rounded-full hover:bg-pink-600 hover:bg-opacity-[45%] focus:outline-none focus:ring-0 transition duration-150 ease-in-out cursor:pointer'>  This Book is rented until {fetchedBook.btime.slice(8,10)}.{fetchedBook.btime.slice(5,7)}.{fetchedBook.btime.slice(0,4)}  </button>}
                Book Details:
